...
public class old_object_code {
	...
	List<String> answer(List<String[]> qt) {
		...
		for (int i = 0; i < qt.size(); i++) {
			...
			if (inqt[0].equals("+")) {
				if (RDL == "") {
					...
					if (active[i - 1][3])
						code.add("ST R,".concat(RDL));// 查询活跃信息表，RDL此时是上一条四元式的结果，如果这个结果变量仍然活跃，才把它弹到内存
					code.add("LD R,".concat(inqt[1]));
					code.add("ADD R,".concat(inqt[2]));
				}
		...
		if (active[qt.size() - 1][3] && (!qt.get(qt.size() - 1)[0].equals("=")))
			code.add("ST R,".concat(RDL));
